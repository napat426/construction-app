...
import { LayoutDashboard, CalendarRange, Package, FileText, ClipboardCheck, ListChecks } from 'lucide-react'
...
export function ProjectTabs({ projectId }: ProjectTabsProps) {
  ...
  const tabs = [
    ...
    },
    {
      ...
      label: 'Check list',
      icon: ListChecks,
      active: pathname.startsWith(`/projects/${projectId}/checklist`),
    },
  ]

  return (
    <div id="project-tabs-nav" className="flex border-b border-slate-200 dark:border-[#1c1c34] mb-6 gap-2 print:hidden no-print">
      {tabs.map((tab) => {
        const Icon = tab.icon
        return (
          <Link
